            ...
            <div className="grid grid-cols-2 gap-3">
              {INSTALLATION_TYPES.map((type) => (
                <label key={type} className="flex items-center space-x-2">
                  <input
                    type="checkbox"
                    checked={profileData.company.installationTypes?.includes(type) || false}
                    onChange={(e) => {
                      const updatedTypes = e.target.checked
                        ? [...(profileData.company.installationTypes || []), type]
                        : (profileData.company.installationTypes || []).filter(t => t !== type);
                      updateCompanyInfo('installationTypes', updatedTypes);
                    }}
              ...
              <div className="flex items-center space-x-4">
                <label className="flex items-center space-x-2">
                  <input
                    type="radio"
                    name="unionPresence"
                    checked={profileData.company.unionPresence === true}
                    onChange={() => updateCompanyInfo('unionPresence', true)}
                ...
                <label className="flex items-center space-x-2">
                  <input
                    type="radio"
                    name="unionPresence"
                    checked={profileData.company.unionPresence === false}
                    onChange={() => {
                      updateCompanyInfo('unionPresence', false);
                      updateCompanyInfo('unionOrganizations', []);
                    }}
                    className="text-blue-600 focus:ring-blue-500"
                  />
                  <span>Non</span>
              ...
              {profileData.company.unionPresence && (
                <div>
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {['FTQ', 'CSN', 'CSQ', 'CSD', 'Syndicat indépendant', 'Autre'].map((union) => (
                      <label key={union} className="flex items-center space-x-2">
                        <input
                          type="checkbox"
                          checked={profileData.company.unionOrganizations?.includes(union) || false}
                          onChange={(e) => {
                            const updatedUnions = e.target.checked
                              ? [...(profileData.company.unionOrganizations || []), union]
                              : (profileData.company.unionOrganizations || []).filter(u => u !== union);
                            updateCompanyInfo('unionOrganizations', updatedUnions);
                          }}
                          className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                        />
